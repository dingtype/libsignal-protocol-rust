pub mod slices {
    ...
    pub fn copy<T: Clone>(
        ...
        dest_pos: usize,
        length: usize,
    ) -> Result<Vec<T>, InvalidSliceError> {
        ...
        if dest_pos > dest.len() - 1 {
            return Err(InvalidSliceError(
                "dest_pos exceeds dest length".to_string(),
            ));
        }
        if length + src_pos > src.len() {
            return Err(InvalidSliceError(
                "Length of dest slice segment exceeds src length.".to_string(),
            ));
        }
        if dest_pos + length > dest.len() {
            return Err(InvalidSliceError(
                "Length of dest slice segment exceeds dest length.".to_string(),
            ));
        }
    ...
    pub fn to_array32(src: &[u8]) -> Result<[u8; 32], InvalidSliceError> {
        match src.try_into() {
            Ok(v) => Ok(v),
            Err(_) => Err(InvalidSliceError(
                "Can't convert slice &[u8] to [u8; 32]".to_string(),
            )),
        }
    }
}
    ...
    pub fn test_copy() {
        let tts = [
            ...
            TestTable {
                ...
                dest_pos: 10,
                length: 11,
                expected: Err(InvalidSliceError(
                    "Length of dest slice segment exceeds src length.".to_string(),
                )),
            },
            TestTable {
